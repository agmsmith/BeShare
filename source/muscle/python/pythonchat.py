...
__author__    = "Jeremy Friesner (jaf@lcsaudio.com)"
__version__   = "$Revision: 1.5 $"
__date__      = "$Date: 2005/01/01 01:03:21 $"
__copyright__ = "Copyright (c) 2005 Level Control Systems"
__license__   = "See enclosed LICENSE.TXT file"

import sys
import Message
import MessageTransceiverThread
import StorageReflectConstants
import select
...
if __name__ == "__main__":
   if len(sys.argv) < 2:
      print "Usage:   python pythonchat.py <servername> [nick]"
      print "Example: python pythonchat.py beshare.tycomsystems.com PythonBoy"
      sys.exit(5)
   ...
   try:
      ...
      mtt = MessageTransceiverThread.MessageTransceiverThread(servername, port)
      ...
      while 1:
         ...
         if notifySocket in inReady:
            ...
            while 1:
               ...
               if nextEvent == MessageTransceiverThread.MTT_EVENT_CONNECTED:
                  ...
                  uploadMsg = Message.Message(StorageReflectConstants.PR_COMMAND_SETDATA)
                  ...
                  subscribeMsg = Message.Message(StorageReflectConstants.PR_COMMAND_SETPARAMETERS)
                  subscribeMsg.PutBool("SUBSCRIBE:beshare/name", 1)
                  mtt.SendOutgoingMessage(subscribeMsg)
               elif nextEvent == MessageTransceiverThread.MTT_EVENT_DISCONNECTED:
                  ...
                  if nextEvent.what == NET_CLIENT_NEW_CHAT_TEXT:
                     ...
                     if fromSession != None:
                        ...
                        nextEvent.PutString(StorageReflectConstants.PR_NAME_KEYS, "/*/"+fromSession+"/beshare");
                  ...
                  elif nextEvent.what == StorageReflectConstants.PR_RESULT_DATAITEMS:
         ...
         if sys.stdin in inReady:
            ...
            if len(usertyped) > 0:
               ...
               chatMsg.PutString(StorageReflectConstants.PR_NAME_KEYS, "/*/*/beshare")
