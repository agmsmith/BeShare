...
class MessageTransceiverThread(threading.Thread):
   ...
   def __init__(self, hostname, port, acceptFrom=None):
      ...
      if self.__hostname == None:
         if acceptFrom == None:
            acceptFrom = ""
         self.__acceptsocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
   ...
   def run(self):
      ...
      try:
         ...
         if self.__acceptsocket == None:
            toremote = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            toremote.setblocking(0)
            try:
               toremote.connect((self.__hostname, self.__port))
   ...
   def __getNextMessageFromMain(self):
      try:
         next = self.__outQ.get(0)
         if next == self._endSession:
            raise self._endSession
         sio = cStringIO.StringIO()
         next.Flatten(sio)
         return (struct.pack("<2L", next.FlattenedSize(), MUSCLE_MESSAGE_ENCODING_DEFAULT), sio.getvalue())
      except Queue.Empty:
...
if __name__ == "__main__":
   ...
   class TestMTT(MessageTransceiverThread):
      ...
      def NotifyMainThread(self):
         while 1:
            next = self.GetNextIncomingEvent()
            if next == MTT_EVENT_CONNECTED:
               print ""
               print "TCP Connection has been successfully created!"
            elif next == MTT_EVENT_DISCONNECTED:
               print ""
