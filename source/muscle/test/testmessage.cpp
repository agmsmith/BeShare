...
#define TEST(x) if ((x) != B_NO_ERROR) printf("Operation failed, line %li\n", __LINE__);
#define NEGATIVETEST(x) if ((x) == B_NO_ERROR) printf("Operation succeeded when it shouldn't have, line %li\n", __LINE__); 
...
int main(int, char **)
{
   ...
   butter.AddTag("Tag", GenericRef(GetMessageFromPool(6666)()));
   butter.AddTag("Tag", GenericRef(GetMessageFromPool(7777)()));
   butter.PrintToStream();
   ...
   msg.AddTag("ThisShouldn'tBeBackAfterUnflatten", GenericRef(NULL));

   const uint8 * getData;
   uint32 getDataSize;
   TEST(msg.FindData("Data", B_RAW_TYPE, (const void **) &getData, &getDataSize));
   String dataStr((const char *) getData, getDataSize);
   printf("getData=[%s], size=%lu\n", dataStr(), getDataSize);

   TEST(msg.FindData("Data", B_RAW_TYPE, 1, (const void **) &getData, &getDataSize));
   dataStr.SetCstr((const char *) getData, getDataSize);
   printf("getData(1)=[%s], size=%lu\n", dataStr(), getDataSize);
  
   printSep("Testing misc");

   printf("There are %i string entries\n", msg.CountNames(B_STRING_TYPE));
   msg.PrintToStream();
   Message tryMe = msg;
   printf("Msg is %lu bytes.\n",msg.FlattenedSize());
   msg.AddTag("anothertag", GenericRef(GetMessageFromPool()()));
   printf("After adding tag, msg is (hopefully still) %lu bytes.\n",msg.FlattenedSize());
   tryMe.PrintToStream();
   ...
   {for (int i=flatSize; i<flatSize*10; i++) buf[i] = 'J';}

   msg.Flatten(buf);

   {for (int i=flatSize; i<flatSize*10; i++) if (buf[i] != 'J') printf("OVERWRITE ON BYTE %i\n",i);}
   printf("\n====\n");
   ...
   else printf("Rats, Unflatten didn't work.  :^(\n");

   {
      printf("Testing field name iterator... B_ANY_TYPE\n");
      MessageFieldNameIterator it = copy.GetFieldNameIterator(B_ANY_TYPE);
      String nextField;
      while(it.GetNextFieldName(nextField) == B_NO_ERROR) 
      {
         printf("--> [%s] (%i)\n",nextField.Cstr(), it.HasMoreFieldNames()); 
      }
   }

   {
      printf("Testing field name iterator... B_STRING_TYPE\n");
      MessageFieldNameIterator it = copy.GetFieldNameIterator(B_STRING_TYPE);
      String nextField;
      while(it.GetNextFieldName(nextField) == B_NO_ERROR) printf("--> [%s] (%i)\n",nextField.Cstr(), it.HasMoreFieldNames()); 
   }

   {
      printf("Testing field name iterator... B_INT8_TYPE\n");
      MessageFieldNameIterator it = copy.GetFieldNameIterator(B_INT8_TYPE);
      String nextField;
      while(it.GetNextFieldName(nextField) == B_NO_ERROR) printf("--> [%s] (%i)\n",nextField.Cstr(), it.HasMoreFieldNames()); 
   }

   {
      ...
      MessageFieldNameIterator it = copy.GetFieldNameIterator(B_OBJECT_TYPE);
      String nextField;
      while(it.GetNextFieldName(nextField) == B_NO_ERROR) printf("--> [%s] (%i)\n",nextField.Cstr(), it.HasMoreFieldNames()); 
   }
   return 0;
