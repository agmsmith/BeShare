...
#endif
   {
...
#endif
	{
         case MUSCLE_THREAD_SIGNAL:
         {
            ...
            String session;
            uint16 port;
            while(_transThread.GetNextEventFromInternalThread(code, &ref, &session, &port) >= 0)
            {
               String codeStr;
               switch(code)
               {
               ...
               printf("/------------------------------------------------------------\n");
               printf("Event from MTT:  type=[%s], session=[%s] port=[%u]\n", codeStr(), session(), port);
...
status_t SetupTransceiverThread(MessageTransceiverThread & mtt, const char * hostName, uint16 port, int method)
{
   switch(method)
   {
      case METHOD_MANUAL:
      {
         int s = Connect(hostName, port, "trc");
         if (s >= 0)
         {
            if (mtt.AddNewSession(s) != B_NO_ERROR)
            {
      ...
      case METHOD_AUTOMATIC:
         ...
         else printf("Will connect asynchronously to [%s:%u]\n", hostName, port);
      break;
         ...
         while(keepGoing)
         {
            ...
            if (len > 0)
            {
               ...
               switch(text[0])
               {
                  ...
                  {
                     ref()->what = PR_COMMAND_SETPARAMETERS;
                     Int32QueryFilter qf("val", Int32QueryFilter::OP_GREATER_THAN, 10);
                     MessageRef filterMsg = GetMessageFromPool();
                     if ((filterMsg())&&(qf.SaveToArchive(*filterMsg()) == B_NO_ERROR)) ref()->AddMessage(&text[2], filterMsg);
                  }
                  break;
                  ...
                  case 'I':
                     ...
                     ref()->AddInt32("br_hostip", GetHostByName("beshare.befaqs.com"));
...
//                  ref.GetItemPointer()->PrintToStream();
      ...
      else printf("Couldn't set up session!\n");
   }
   else
   {
