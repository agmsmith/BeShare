...
int main(int argc, char ** argv) 
{
   ...
   if (m.SetArea(TEST_KEY, TEST_AREA_SIZE, true) == B_NO_ERROR)
   {
      ...
      else
      {
         ...
         LogTime(MUSCLE_LOG_INFO, "Area is %lu bytes long, starting at address %p\n", s, a);
         ...
         while(1)
         {
            if (OnceEvery(1000000, lastTime)) LogTime(MUSCLE_LOG_INFO, "Still going... base=%u\n", base);
            ...
            if (m.LockAreaReadWrite() == B_NO_ERROR)
            {
               ...
               {
                  uint8 * a = m.GetAreaPointer();
                  {for (uint32 i=1; i<s; i++) if (a[i-1] != a[i]) LogTime(MUSCLE_LOG_ERROR, "A. ERROR@%lu\n",i);}
                  {for (uint32 i=0; i<s; i++) a[i] = base;}
                  {for (uint32 i=0; i<s; i++) if (a[i] != base) LogTime(MUSCLE_LOG_ERROR, "B. ERROR@%lu\n",i);}
               }
            ...
            if (m.LockAreaReadOnly() == B_NO_ERROR)
            {
               ...
               if (s > 0)
               {
                  ...
                  {for (uint32 i=1; i<s; i++) if (a[i-1] != a[i]) LogTime(MUSCLE_LOG_ERROR, "C. ERROR@%lu\n",i);}
               }
               else LogTime(MUSCLE_LOG_ERROR, "Area size is zero!?\n");
