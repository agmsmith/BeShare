...
package com.lcs.muscle.message;
...
import com.lcs.muscle.support.LEDataOutputStream;
import com.lcs.muscle.support.Point;
import com.lcs.muscle.support.Rect;
...
public final class Message implements Flattenable
{
   ...
   public void flatten(ByteBuffer out) throws IOException 
   {
      ...
      while(e.hasMoreElements())
      {
         ...
         out.put(nameBytes);
         out.put((byte)0);  // terminating NUL byte
         out.putInt(field.typeCode());
         out.putInt(field.flattenedSize());
         field.flatten(out);
      }
   }
   

   public void unflatten(ByteBuffer in, int numBytes) throws UnflattenFormatException, IOException 
   {
      // Now do the actual work on the byte buffer.
      clear();
      ...
      what = in.getInt();
      int numEntries = in.getInt();
      byte [] stringBuf = null;
      if (numEntries > 0) ensureFieldTableAllocated();
      for (int i=0; i<numEntries; i++)
      {
         int fieldNameLength = in.getInt();
         if ((stringBuf == null)||(stringBuf.length < fieldNameLength)) stringBuf = new byte[((fieldNameLength>5)?fieldNameLength:5)*2];
         in.get(stringBuf, 0, fieldNameLength);
         String fieldName = new String(stringBuf, 0, fieldNameLength-1);
         MessageField field = getCreateOrReplaceField(fieldName, in.getInt());
         field.unflatten(in, in.getInt());
         _fieldTable.put(fieldName, field);
      }
   }
   ...
   private final class MessageField implements Flattenable
   {
      ...
      public void flatten(ByteBuffer out) throws IOException
      {
         switch(typeCode())
         {
            ...
            { 
               boolean [] array = (boolean[]) _payload;
               for (int i=0; i<_numItems; i++) out.put(array[i] ? (byte)1 : (byte)0);
            }
            break;
            ...
            case B_MESSAGE_TYPE:
            {
               ...
               for (int i=0; i<_numItems; i++)
               {
                  out.putInt(array[i].flattenedSize());
                  array[i].flatten(out);
               }
            }
            break;
            ...
            {
               String array[] = (String[]) _payload;
               out.putInt(_numItems);
               for (int i=0; i<_numItems; i++) 
               {
                  try {
                     byte[] utf8Bytes = array[i].getBytes("UTF8");
                     out.putInt(utf8Bytes.length + 1);
                     out.put(utf8Bytes);
                  } catch (UnsupportedEncodingException uee) {
                     out.putInt(array[i].length() + 1);
                     out.put(array[i].getBytes());
                  }
                  out.put((byte)0);  // nul terminator
               }
            }
            break;
      ...
      public void unflatten(ByteBuffer in, int numBytes) throws IOException, UnflattenFormatException
      {
          ...
          switch(_type)
          {
             case B_BOOL_TYPE:  
             {
                ...
                for (int i=0; i<_numItems; i++) array[i] = (in.get() > 0);
                _payload = array;
             }
             break;
             ...
             case B_INT8_TYPE:             
             {
                ...
                in.get(array, 0, _numItems);  // wow, easy
                _payload = array;
             }
             break;
...
//              for (int j=0; j<_numItems; j++) array[j] = (Message) temp.get(j);
                Message[] array = (Message[]) temp.toArray(new Message[_numItems]);
                _payload = array;
             }
             break;
