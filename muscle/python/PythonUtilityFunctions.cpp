...
static const char * fname(const String & key, const char * defaultFieldName) {return (key.Length() > 0) ? key() : defaultFieldName;}
   ...
   if (msg.GetInfo(fieldName, &type) == B_NO_ERROR)
   {
      ...
      switch(type)
      {
         ...
         case B_RAW_TYPE: 
         {
            const char * temp;
            uint32 size;
            if (msg.FindData(fieldName, index, B_RAW_TYPE, (const void **)&temp, &size) == B_NO_ERROR) return PyString_FromStringAndSize(temp, (int)size);
         }
         break;
         ...
         case B_MESSAGE_TYPE:
         {
            ...
            if (msg.FindMessage(fieldName, index, subMsg) == B_NO_ERROR)
            {
               ...
               if (ret)
               {
                  MessageFieldNameIterator iter = subMsg()->GetFieldNameIterator(B_ANY_TYPE, HTIT_FLAG_NOREGISTER);
                  const String * fn; 
                  while((fn = iter()) != NULL)
                  {
                     uint32 j = 0;
                     while(true)
                     {
                        PyObject * sub = ConvertMessageItemToPyObject(*subMsg(), *fn, j++);
                        if (sub)
                        {
                           ...
                           {
                              if (PyDict_SetItemString(ret, (char *) fn->Cstr(), sub) != 0) Py_DECREF(sub);
                           }
                        }
                        else break;
                     }
         ...
         default:
            PyErr_SetString(PyExc_RuntimeError, "Message contained unsupported datatype");
            setErr = true;
         break;
      }
      if (setErr == false) PyErr_SetString(PyExc_RuntimeError, "Message item not found");
   }
   else PyErr_SetString(PyExc_RuntimeError, "Malformed Message");
