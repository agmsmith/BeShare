...
class Message:
   ...
   def PrintToStream(self):
      ...
      print "Message: what =", self.what, "FlattenedSize =", self.FlattenedSize()
      for x in self.__fields.keys():
         print " [", x, ":", GetHumanReadableTypeString(self.GetFieldType(x)), ":", self.GetFieldContents(x), "]"
   ...
   def FlattenedSize(self):
      ...
      for fieldName in fields:
         sum += 4+len(fieldName)+1+4+4  # namelen(4), name(n), NUL(1), type(4), fieldBytes(4)
         fieldContents = self.GetFieldContents(fieldName)
         fieldType     = self.GetFieldType(fieldName)
         sum += self.GetFieldContentsLength(fieldType, fieldContents)
      return sum
   ...
   def Flatten(self, file):
      ...
      for fieldName in self.__fields.keys():
         ...
         if type(fieldContents) == array.ArrayType:
            if fieldType == B_BOOL_TYPE or fieldType == B_DOUBLE_TYPE or fieldType == B_FLOAT_TYPE or fieldType == B_INT32_TYPE or fieldType == B_INT16_TYPE or fieldType == B_INT8_TYPE:
   ...
   def GetFieldContentsLength(self, fieldType, fieldContents):
      ...
      if fieldType == B_BOOL_TYPE:
         ...
         sum += itemCount*4  # item length fields (4 each) (no num_items field for this type!)
         for item in fieldContents:
            sum += item.FlattenedSize()
      elif fieldType == B_POINT_TYPE:
         sum += itemCount*8   # 2 floats per point
      elif fieldType == B_RECT_TYPE:
         sum += itemCount*16  # 4 floats per rect
      elif fieldType == B_STRING_TYPE:
         sum += 4  # num_strings(4)
         for item in fieldContents:
            sum += 4+len(item)+1  # string_length(4), string(n), NUL(1)
      else:
         sum += 4  # num_bufs(4)
         for item in fieldContents:
            sum += 4+len(item)
...
if __name__ == "__main__":
   ...
   tm.PrintToStream()
   outfile = open('test.msg', 'w')
   tm.Flatten(outfile)
   outfile.close()

   print "Unflattening..."
   infile = open('test.msg', 'r')
   m2 = Message()
   m2.Unflatten(infile);
   infile.close()
   m2.PrintToStream()
